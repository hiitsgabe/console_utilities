...
class TouchState:
    ...
    scroll_accumulated: float = 0  # Accumulated scroll in pixels
    last_click_time: int = 0
    last_clicked_item: int = -1
    velocity_y: float = 0  # For momentum scrolling
...
class TouchHandler:
    ...
    def __init__(self):
        ...
        try:
            ...
            if touch_device_count > 0:
                ...
                print(
                    f"Touchscreen detected: {touch_device_count} touch device(s) available"
                )
            else:
    ...
    def handle_finger_up(
        ...
        if (
            ...
            and time_elapsed < TAP_TIME_THRESHOLD
            and not self._state.is_scrolling
        ):
        ...
        self._state.is_scrolling = False
        self._state.scroll_accumulated = 0
    ...
    def handle_finger_motion(
        ...
        Returns:
            Scroll amount in items (positive = up, negative = down)
        """
        ...
        # Calculate motion from last position (in pixels)
        ...
        if self._state.is_scrolling:
            # Accumulate scroll distance (negative because drag down = scroll up)
            self._state.scroll_accumulated -= dy_motion

            # Convert accumulated pixels to items
            item_height = SCROLL_ITEM_HEIGHT
            while abs(self._state.scroll_accumulated) >= item_height:
                if self._state.scroll_accumulated > 0:
                    scroll_items += 1
                    self._state.scroll_accumulated -= item_height
                else:
                    scroll_items -= 1
                    self._state.scroll_accumulated += item_height

            # Trigger scroll callback for each item
            if scroll_items != 0 and on_scroll:
                on_scroll(scroll_items)
    ...
    def check_double_click(self, item_index: int) -> bool:
        ...
        is_double = (
            item_index == self._state.last_clicked_item
            and current_time - self._state.last_click_time < DOUBLE_CLICK_THRESHOLD
        )
