...
Everything except `WePatcher` and `EspnClient` is re-exported straight from the
library, so the names `app.py` imports from this package resolve to the same
classes the library's own patcher uses.
"""
