...
class FileExplorerScreen:
    ...
    def render(
        ...
        self.text.render(
            ...
            breadcrumb_text,
            (
                inset + self.theme.padding_md,
                breadcrumb_y + (breadcrumb_h - font_sm) // 2,
            ),
    ...
    def _render_context_menu(self, screen, fe, rects):
        ...
        for i, (action_id, label) in enumerate(actions):
            ...
            self.text.render(
                screen,
                label,
                (menu_x + 20, item_y + (item_h - self.theme.font_size_md) // 2),
                color=text_color,
                size=self.theme.font_size_md,
            )
    ...
    def _render_viewer(self, screen, fe, rects):
        ...
        if len(fe.viewer_content) > visible_lines:
            ...
            pygame.draw.rect(
                screen, self.theme.text_secondary, bar_rect, border_radius=2
            )
    ...
    def _render_extract_modal(self, screen, fe, rects):
        ...
        for i, label in enumerate(options):
            ...
            self.text.render(
                screen,
                label,
                (opt_rect.left + 12, opt_rect.centery - self.theme.font_size_md // 2),
                color=color,
                size=self.theme.font_size_md,
            )
    ...
    def _render_input_modal(self, screen, fe, input_mode, rects):
        ...
        kb_rect = pygame.Rect(
            modal_rect.left + 10, input_y + 40, modal_rect.width - 20, modal_h - 100
        )
